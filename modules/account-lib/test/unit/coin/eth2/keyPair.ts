...
describe('Eth2 Key Pair', () => {
  describe('should create a valid KeyPair', () => {
    ...
    it('from a byte array private key directly', () => {
      const privateKey = Buffer.from(testData.ACCOUNT_1.privateKeyBytes);
      const keyPair = new KeyPair();
      keyPair.recordKeysFromPrivateKeyBytes(privateKey);
      should.equal(keyPair.getKeys().prv, '0x' + privateKey.toString('hex'));
    });
